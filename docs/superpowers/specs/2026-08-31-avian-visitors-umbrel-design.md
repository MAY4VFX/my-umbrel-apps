...
3. **Аудио-архив на SSD.** Записи птиц живут в именованном томе `birdsongs`. На этом
   хосте `/var/lib/docker` и `~/umbrel` примонтированы с внешнего SSD, поэтому
   отдельные пути в `/mnt/data` не нужны (а их создание требовало root). Том, в
   отличие от bind-mount, наследует владельца из образа — иначе каталог создался бы
   от root и BirdNET не смог бы в него писать. На SD-карту запись не идёт.
